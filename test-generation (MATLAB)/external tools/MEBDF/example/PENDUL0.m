%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
% DRIVER ROUTINE FOR THE PENDULUM PROBLEM: INDEX = 0.   
...
LOUT = fopen('pendul0.txt','w');

 fprintf(LOUT,'\r COMPUTATIONAL STATISTICS OF THE PENDUL0 PROBLEM USING MEBDFAE \r');
...
%disp('now in F')
      ...
      DF(3)=-Y(1)*Y(5);
      DF(4)=-Y(2)*Y(5)-1;
      DF(5) = -2*(Y(1)*Y(3)+Y(2)*Y(4))*Y(5)-3*Y(4);
