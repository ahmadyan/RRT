%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
% DRIVER ROUTINE FOR THE B550 EQUATION.   
    ...
    while (MYtag == 0)
        ...
        switch MYcase
           ...
           [N,X,H,Y,XOUT,XEND,MF,INDEX,LOUT,LWORK,WORK_1,WORK_2,WORK_3,...
               ...
               = MEBDF(N,X,H,Y,XOUT,XEND,MF,INDEX,LOUT,LWORK,WORK_1,...
               WORK_2,WORK_3,WORK_I1,WORK_I2,WORK_I3,WORK_I4,WORK_I5,...
               WORK_I6,WORK_I7,WORK_I8,WORK_I9,WORK_I10,WORK_I11,LIWORK,...
               IWORK,IWORK_15,MBND,MASBND,MAXDER,ITOL,RTOL,ATOL,RPAR,IPAR,IERR);  
           ...
           continue
            ...
            case 995
                ...
                XOUT = X;
                TRUE(1)=exp(-10*XOUT)*(cos(50*XOUT)+sin(50*XOUT));
                TRUE(2)=exp(-10*XOUT)*(cos(50*XOUT)-sin(50*XOUT));
                TRUE(3)=exp(-4*XOUT);
                TRUE(4)=exp(-XOUT);
                TRUE(5)=exp(-0.5*XOUT);
                TRUE(6)=exp(-0.1*XOUT);  
