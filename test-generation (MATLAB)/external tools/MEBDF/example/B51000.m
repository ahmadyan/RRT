%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
% DRIVER ROUTINE FOR THE B51000 EQUATION.   
         ...
         [N,X,H,Y,XOUT,XEND,MF,INDEX,LOUT,LWORK,WORK_1,WORK_2,WORK_3,...
           ...
           continue
            ...
            case 995
                ...
                XOUT = X;
                TRUE(1)=exp(-10*XOUT)*(cos(1000*XOUT)+sin(1000*XOUT));
                TRUE(2)=exp(-10*XOUT)*(cos(1000*XOUT)-sin(1000*XOUT));
                TRUE(3)=exp(-4*XOUT);
                TRUE(4)=exp(-XOUT);
                TRUE(5)=exp(-0.5*XOUT);
                TRUE(6)=exp(-0.1*XOUT);  
...
%disp('now in F')
      ...
      DF(1)=-10*Y(1)+1000*Y(2);
      DF(2)=-1000*Y(1)-10*Y(2);
      DF(3)=-4*Y(3);
      DF(4)=-Y(4);
      DF(5)=-0.5*Y(5);
      DF(6)=-0.1*Y(6);
