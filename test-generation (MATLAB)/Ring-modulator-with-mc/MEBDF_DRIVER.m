...
%STANDARD SOLVER.    
   ...
   [N,LWORK,LIWORK,MBND,MASBND,RPAR,IPAR,IERR,IWORK_15,...
    ...
    [N,X,H,Y,XOUT,XEND,MF,INDEX,LWORK,WORK_1,WORK_2,WORK_3,...
                    ...
                    LWORK,WORK_1,WORK_2,WORK_3,WORK_I1,WORK_I2,...
...
if strcmp(stat_on_or_off,'on')
    ...
    printf('\r\r COMPUTATIONAL STATISTICS OF RUN');
    printf('\r\r NUMBER OF EQUATIONS: %g',N);
    printf('\r\r RESULTS WITH THE FOLLOWING TOL :');
    printf('\r RTOL = %E',RTOL);
    printf('\r ATOL = %E',ATOL);
    printf('\r\r\r LAST STEP SIZE: %E',HUSED);
    printf('\r LAST ORDER OF THE METHOD: %g',NQUSED);
    printf('\r MAXIMUM ORDER USED SO FAR: %g',MAXORD);
    printf('\r TOTAL NUMBER OF STEPS TAKEN: %g',NSTEP);
    printf('\r TOTAL NUMBER OF FAILED STEPS: %g',NFAIL);
    printf('\r NUMBER OF FUNCTION EVALUATIONS: %g',NFE);
    printf('\r NUMBER OF JACOBIAN EVALUATIONS: %g',NJE);
    printf('\r NUMBER OF FACTORIZATION: %g',NDEC);
    printf('\r NUMBER OF BACKSOLVES: %g',NBSOL);
    printf('\r CPU TIME: %g',TIME);
...
%**************************************************************************
%**************************************************************************
function  [N,X,H,Y,XOUT,XEND,MF,INDEX,LWORK,WORK_1,WORK_2,WORK_3,...
                    WORK_I1,WORK_I2,WORK_I3,WORK_I4,WORK_I5,WORK_I6,...
                    WORK_I7,WORK_I8,WORK_I9,WORK_I10,WORK_I11,LIWORK,...
                    IWORK,IWORK_15,MBND,MASBND,MAXDER,ITOL,RTOL,ATOL,...
                    RPAR,IPAR,IERR,Y_RETURN,X_RETURN,TIME]= ...
...
if (IDID == 1)
    ...
    if (N <= 0)
        ...
        printf(['*** ERROR ***** INTEGRATION HALTED IN DRIVER \n >>>' ...
...
elseif (IDID==3)
    ...
    if ((ITOL==1) && (RTOL(1)== 0))
        ...
        printf(' ILLEGAL INPUT.. RTOL .LE. 0. \n');
        %WRITE(LOUT,9040)
        IDID = -4;
    end
...
while (MYstop == 0)
    ...
    switch MYcase
        ...
        case 8000
            ...
            if (IERR~=0)
                printf(['IERR IS NON-ZERO BECAUSE OF AN ILLEGAL' ...
                    ' FUNCTION CALL']);
                %write(LOUT,1975)             
                h= h/2;
                if (H<EPSJAC/100)
